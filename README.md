...
v1.8.1 起，脚本打开 ShopCity Facebook 转化页面后会自动识别当前登录店铺的 Shop ID。识别顺序为登录状态接口、当前网址、页面数据、Cookie 和浏览器存储。手动填写仍保留为兜底；自动检测到的值与现有手动值不同时，不会在后台静默覆盖，可点击面板中的“自动获取”按钮主动替换。
